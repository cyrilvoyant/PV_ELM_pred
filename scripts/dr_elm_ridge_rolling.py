"""
ELM Ridge in ROLLING "full-window" mode.
...
ELM Ridge (beta = (H^T H + lam I)^-1 H^T Y) is trained on ALL supervised pairs of
the window data[T-W:T] (contiguous targets, ~1488 pts for 1 month): any pair
whose LB inputs (ending FH steps before the target) AND the target fit inside
the window.
...
The hyperparameter lam_r is frozen ONCE per (window, FH) via temporal
...
# Clip predictions to >=0 only for physically non-negative quantities (PV, GHI);
# for non-solar sets (temperature) CLIP_NONNEG is False -> identity (allow < 0).
_clip = (lambda a: np.clip(a, 0.0, None)) if CLIP_NONNEG else (lambda a: a)


def _h_to_steps(hours):
    """Physical horizons (h) -> step counts at the dataset step size.
    0.5/1/3/6/10 h -> {1,2,6,12,20} at 48 steps/day, {3,6,18,36,60} at 144."""
    return [round(h * STEPS_PER_DAY / 24.0) for h in hours]


def time_features_at(idx_abs, steps_per_day=STEPS_PER_DAY):
...
LB = STEPS_PER_DAY            # 24 h lookback
SEED = 42
...
# Windows in steps (STEPS_PER_DAY steps/day).
WINDOWS = {"1m": 31 * STEPS_PER_DAY, "6m": 183 * STEPS_PER_DAY, "1y": 365 * STEPS_PER_DAY}
...
if SMOKE_TEST:
    ...
    N_candidates = 2
    CV_FOLDS = 3
    N_TEST_DAYS = 5        # SMOKE: truncate the test to 5 days
    WINDOWS = {"1m": 31 * STEPS_PER_DAY, "3m": 90 * STEPS_PER_DAY}   # reduced windows
    Ndata = round(8000 * STEPS_PER_DAY / 48)
else:
    print("*** FULL MODE ***")
...
def build_window(data, T, W, FH, mu=None, sd=None):
    ...
    X_win = np.concatenate(
        [(X_raw - mu) / sd, time_features_at(tgt, STEPS_PER_DAY)], axis=1
    )
...
def predict_input(data, T, FH, mu, sd):
    ...
    return np.concatenate(
        [((x_pred_raw - mu) / sd)[None, :], time_features_at([T], STEPS_PER_DAY)],
        axis=1,
    )


def select_hparams_for_window(data, test_start, W, FH, rng):
    """Freeze lam_r by temporal expanding-window CV on the warm-up block.
    ...
    contiguous fit). Criterion = validation RMSE (Y scale, after clip) summed
    over all folds. IW/bias drawn once (rng); mu/sd on the whole warm-up block.
    """
    in_size = LB + 4
    ...
    sse = {lr: 0.0 for lr in LAMBDA_GRID}
    n_val = 0
    for fit, val in temporal_cv_splits(len(tgt), CV_FOLDS):
        ...
        for lam_r in LAMBDA_GRID:
            beta = ridge_solve(Hf, y[fit], lam_r)
            yv = _clip(Hv @ beta)
            sse[lam_r] += float(np.sum((yv - y[val]) ** 2))
    lam_r = min(LAMBDA_GRID, key=lambda lr: sse[lr])
    rmse = sqrt(sse[lam_r] / n_val)
    print(f"    [hparams W={W} FH={FH}] lam_r={lam_r:g} val_RMSE={rmse:.4g}")
    return lam_r


def select_hidden_for_window(X_win, y_win, lam_r, rng):
    ...
    for _ in range(N_candidates):
        ...
        for fit, val in splits:
            ...
            beta = ridge_solve(Hf, y_win[fit], lam_r)
    ...
    beta = ridge_solve(H, y_win, lam_r)
...
def roll_one_window(data, is_day_full, test_indices, W, win_label, rng_seed_base):
    ...
    step. lam_r and mu/sd frozen per FH.
    """
    rows, pred_rows = [], []
    for FH in FH_list:
        lam_r = select_hparams_for_window(
        ...
        extra = {"N_params": N_hidden * (LB + 4) + 2 * N_hidden,
                 "window": win_label, "lambda_ridge": lam_r,
        ...
        rows.append(build_metric_row(
            ...
            extra_fields={**extra, "N_params": 0, "lambda_ridge": np.nan},
        ))
        for m, yp in ((method, y_pred), ("Persistence_P", y_persis)):
            pred_rows.append(pd.DataFrame({
                "Method": m, "LB_days": LB / STEPS_PER_DAY,
...
def main():
    ...
    if N_TEST_DAYS is not None:
        test_indices = test_indices[: N_TEST_DAYS * STEPS_PER_DAY]
    ...
    extra_cols = ["N_params", "window", "lambda_ridge", "refit_every"]
    ...
    for win_label, W in WINDOWS.items():
        print(f"\n===== WINDOW {win_label} (W={W} steps, {W/STEPS_PER_DAY:g} d) =====")
        ...
        out_all = RESULTS_DIR / f"Results_DR_ELM_ridge_rolling_{win_label}{suffix}_all.csv"
        out_day = RESULTS_DIR / f"Results_DR_ELM_ridge_rolling_{win_label}{suffix}_day.csv"
        df_all, df_day = split_and_save(rows, extra_cols, str(out_all), str(out_day))
        print(df_all.to_string())
        print(f"  -> {out_all}")
        pred = pd.concat(pred_rows, ignore_index=True)
        pred_file = RESULTS_DIR / f"Predictions_DR_ELM_ridge_rolling_{win_label}{suffix}.csv"
...
if __name__ == "__main__":
    ...
    main()
    dt = time.time() - t0
    print(f"\n[chrono] dr_elm_ridge_rolling.py : {dt:.2f} s  ({dt/60:.2f} min)")

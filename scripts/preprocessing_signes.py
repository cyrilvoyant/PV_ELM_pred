"""
Preprocessing of the Signes PV dataset.

Reads `data/cs/signes.csv` (columns `measure_date`, `prod`; 5-min step;
730 kWp rooftop plant near Signes, France, lat 43.25551 / lon 5.8),
resamples the production into 30-minute averages and saves the resulting
1D array to `data/cs/signes_30min.npy`.

Same two differences with the Palaiseau series as Oxelar:

  * The raw series does NOT start at midnight (first timestamp is
    2023-10-31 08:00 UTC). The whole pipeline assumes arr[0] is the
  ...
  * Timestamps are end-of-interval vs start-of-interval for Palaiseau.
    We do NOT shift: a uniform shift is invisible to the AR/persistence/
    NICE machinery and the residual offset is below the 30-min resolution.

In addition, ~48% of the raw rows have an empty `prod` field: these are the
nighttime slots (blanks start ~sunset, resume ~sunrise, tracking the seasonal
daylight cycle). Treating them as 0 is correct (night = zero production), and
load_30min already maps NaN -> 0, so no special handling is needed here.

The unit is kept as-is; NICE / nRMSE / R2 are dimensionless and remain
...
CSV_FILE = os.path.join(PROJECT_ROOT, 'data', 'cs', 'signes.csv')
CACHE_NPY = os.path.join(PROJECT_ROOT, 'data', 'cs', 'signes_30min.npy')
...
if __name__ == '__main__':
    ...
    main()
    dt = time.time() - t0
    print(f'\n[chrono] preprocessing_signes.py : {dt:.2f} s  ({dt/60:.2f} min)')
